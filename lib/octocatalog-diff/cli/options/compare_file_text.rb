...
# set --compare-file-text=force or --compare-file-text=forcenoexc. These options allow
# the content of the file to be substituted in to --catalog-only compilations, for example.
# 'force' will raise an exception if the underlying file can't be found; 'forcenoexc' won't.
#
...
OctocatalogDiff::Cli::Options::Option.newoption(:compare_file_text) do
  ...
  def parse(parser, options)
    parser.on('--[no-]compare-file-text[=force|forcenoexc]', 'Compare text, not source location, of file resources') do |x|
      if x == 'force' || x == 'forcenoexc'
