...
require OctocatalogDiff::Spec.require_path('/external/pson/common')
require OctocatalogDiff::Spec.require_path('/external/pson/pure')
...
describe PSON do
  ...
  it 'should be able to handle arbitrary binary data' do
    bin_string = (1..20_000).collect { |i| ((17 * i + 13 * i * i) % 255).chr }.join
    parsed = PSON.parse(%({ "type": "foo", "data": #{bin_string.to_pson} }))['data']
