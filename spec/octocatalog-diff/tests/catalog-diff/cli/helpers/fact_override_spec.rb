...
describe OctocatalogDiff::Cli::Helpers::FactOverride do
  ...
  context 'with proper typed input' do
    describe '#new' do
      it 'should pass through a string' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(string)chicken')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('chicken')
      end

      it 'should pass through an empty string' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(string)')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('')
      end

      it 'should pass through a multi-line string' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new("foo=(string)foo\nbar")
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq("foo\nbar")
      end

      it 'should pass through a positive integer' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(fixnum)42')
        expect(testobj.key).to eq('foo')
      ...
      it 'should pass through a positive float' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(float)42.15')
        expect(testobj.key).to eq('foo')
      ...
      it 'should pass through a positive float with no leading digit' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(float).15')
        expect(testobj.key).to eq('foo')
      ...
      it 'should handle true boolean' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(boolean)true')
        expect(testobj.key).to eq('foo')
      ...
      it 'should handle false boolean' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(boolean)false')
        expect(testobj.key).to eq('foo')
      ...
      it 'should parse JSON' do
        ...
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new(arg)
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('bar' => 'baz')
      end

      it 'should return string even when input looks like a number' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(string)42')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('42')
      end

      it 'should return string even when input looks like a float' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(string)42.15')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('42.15')
      end

      it 'should return string even when input looks like a boolean' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(string)true')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('true')
      end

      it 'shouuld pass through a nil with no argument' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(nil)')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(nil)
      end

      it 'should pass through a nil with superfluous argument' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=(nil)blahblahblah')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(nil)
      end
    end
  ...
  context 'with proper guessed string input' do
    describe '#new' do
      it 'should pass through a string' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=chicken')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('chicken')
      end

      it 'should pass through an empty string' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq('')
      end

      it 'should pass through a multi-line string' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new("foo=foo\nbar")
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq("foo\nbar")
      end

      it 'should pass through a positive integer' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=42')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(42)
      end
      ...
      it 'should pass through a positive float' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=42.15')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(42.15)
      end
      ...
      it 'should pass through a positive float with no leading digit' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=.15')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(0.15)
      end
      ...
      it 'should handle true boolean' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=true')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(true)
      end
      ...
      it 'should handle false boolean' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new('foo=false')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(false)
      end
  ...
  context 'with non-string input' do
    describe '#new' do
      it 'should pass through a fixnum without manipulation' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new(42, 'foo')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(42)
      end

      it 'should pass through a float without manipulation' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new(42.15, 'foo')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(42.15)
      end

      it 'should pass through a boolean without manipulation' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new(false, 'foo')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(false)
      end

      it 'should pass through a nil without manipulation' do
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new(nil, 'foo')
        expect(testobj.key).to eq('foo')
        expect(testobj.value).to eq(nil)
      end
      ...
      it 'should pass through a hash without manipulation' do
        ...
        testobj = OctocatalogDiff::Cli::Helpers::FactOverride.new(arg, 'foo')
        expect(testobj.key).to eq('foo')
